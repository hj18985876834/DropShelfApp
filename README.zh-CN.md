...
- 设置项：收纳栏位置、主题、密度、语言、开机自启动。
- 检查更新：手动检查更新、下载安装包、校验 SHA-256、启动安装器。
...
当发现新版本时，EdgeTuck 会下载 `EdgeTuckSetup.exe`，根据清单中的 SHA-256 校验安装包，校验通过后启动安装器并退出当前运行的应用。检查更新成功后，也会根据清单中的品牌信息刷新设置页的软件名称和介绍。更新由用户在设置中手动触发；应用不需要账号，也不会进行静默后台自动更新。
...
卸载器会移除已安装的程序文件和快捷方式。会移除 `EdgeTuck` 开机自启动 Run 值。对于旧版本升级场景，如果存在旧的 `DropShelf` Run 值，也会一并移除。
