...
- 批量导入：可从剪贴板粘贴多条结构化路径或链接，一次加入多个文件、文件夹、图片或链接卡片。
- 拖出复用：支持把暂存卡片拖回资源管理器、桌面、编辑器、聊天窗口或其它支持拖放的应用。
- 卡片管理：搜索卡片、按类型筛选、多选当前可见结果，并可拖动类型图标区域调整卡片顺序。
- 快捷操作：复制、打开、在资源管理器中定位、移除单项、清空全部记录。
- 快速访问：可用全局快捷键显示/隐藏收纳栏，或在后台把当前剪贴板内容加入收纳栏。
...
4. 在其它应用中复制文本、URL 或截图后，粘贴到 EdgeTuck。
   如果剪贴板文本包含多条有效路径或链接，会分别加入为多个卡片。
5. 使用卡片上的操作按钮：
   - `复制`：复制路径、文本、URL 或图片。
   - `打开`：使用 Windows 默认方式打开文件、文件夹、URL 或图片。
   - `在资源管理器中定位`：在资源管理器中显示文件、文件夹或图片所在位置。
   - `移除`：移除该暂存记录。
6. 使用筛选控件和搜索框缩小当前可见卡片范围。
7. 使用 `Ctrl+点击`、`Shift+点击` 或 `Ctrl+A` 多选当前可见卡片。混合类型多选复制不支持，EdgeTuck 会提示先选择同一类型；文件、文件夹、图片、文本、链接会按各自类型复制。
8. 拖动卡片的类型图标区域，在收纳栏内调整卡片顺序；被拖动卡片会呈现悬浮效果，拖动过程中卡片会动态移动。
9. 需要复用时，从卡片正文区域将卡片拖出到其它应用。
...
- `Enter`：打开当前选中项。
- `Ctrl+C`：复制当前选中项。
- `Ctrl+点击`：将当前可见卡片加入或移出选择。
- `Shift+点击`：选择一段当前可见卡片。
- `Ctrl+A`：选择当前全部可见卡片。
- `Ctrl+Alt+Space`：全局显示或隐藏 EdgeTuck。
- `Ctrl+Alt+V`：在后台把当前剪贴板内容加入 EdgeTuck。
- 双击托盘图标：显示收纳栏。
