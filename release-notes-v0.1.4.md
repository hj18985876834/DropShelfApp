...
- 新增卡片搜索，可和类型筛选一起使用，更快找到暂存内容。
- 支持从剪贴板批量导入多条有效路径或链接，并按类型显示加入结果。
- 支持多选当前可见卡片，可批量移除或复制。
- 批量移除前会确认，原始文件和文件夹不会被删除。
...
- Added card search that works together with type filtering.
- Added batch import from clipboard text containing multiple valid paths or links, with type-specific add feedback.
- Added multi-select for visible cards with batch remove and copy.
- Batch remove asks for confirmation and never deletes original files or folders.
