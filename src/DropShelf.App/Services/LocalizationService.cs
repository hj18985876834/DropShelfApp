    ...
    string ReorderHandleTooltip)
{
    public static AppText Chinese { get; } = new(
        ...
        "应用",
        "关闭",
        "添加内容：拖放或粘贴文件、文件夹、图片、文本和链接。\n批量导入：多行路径或链接会分别创建多条记录。\n查找整理：可搜索、筛选，并使用 Ctrl 或 Shift 选择多个项目。\n快捷操作：Ctrl+Alt+Space 显示或隐藏收纳栏，Ctrl+Alt+V 添加当前剪贴板内容。\n删除记录：只移除收纳记录，不会删除原始文件。",
        "江江学长",
        "设置已保存。",
        "无法保存设置。",
        " 项",
    ...
    public static AppText English { get; } = new(
        ...
        "Apply",
        "Close",
        "Add content: drag or paste files, folders, images, text, and links.\nBatch import: multiple paths or links are added as separate items.\nFind and organize: search, filter, and select multiple items with Ctrl or Shift.\nQuick actions: Ctrl+Alt+Space shows or hides the shelf, and Ctrl+Alt+V adds the current clipboard content.\nRemove records: removing an item only removes it from the shelf and does not delete the original file.",
        "Jiangjiang Xuezhang",
        "Settings saved.",
